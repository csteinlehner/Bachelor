...
CSV_FILE_PATH = File.join(File.dirname(__FILE__), "../../../data/csv/instagram_top_color/instagram_top_color_time_#{city}__#{Time.new.strftime("%Y-%m-%d_%H-%M-%S")}.csv")
...
path = "/Users/norrix/fhp/Bachelor/PROCESS/data/insta_dl/"
# dir = Dir["#{path}#{city}_*"]
# dir.each do |f|
...
FasterCSV.open(CSV_FILE_PATH, "w") do |csv|
	...
	while t_start <= end_range do
		...
		if(img_list.length>0)
			colors = img_list.quantize(4,RGBColorspace,NoDitherMethod)
			a_colors = colors.append(true)
			# a_colors = a_colors.blur_image(20,20)
			# a_colors.write("/Users/norrix/fhp/Bachelor/PROCESS/data/tmp/#{tz.utc_to_local(t_start)}.jpg")
