...
(1..(steps_lat+1)).each do |i|
  ...
  (1..(steps_lng+1)).each do |j|
    ...
    csv << t_midpoint.to_a + [size] + [array_to_csvstring(language_count.sort_by{|k,v| v}.reverse)] + [array_to_csvstring(user_count.sort_by {|k,v| v}.reverse[0..9])] + [array_to_csvstring(word_count(words_all_rows,5))]
