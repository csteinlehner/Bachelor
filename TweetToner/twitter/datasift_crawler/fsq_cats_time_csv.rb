...
FasterCSV.open(CSV_FILE_PATH, "w") do |csv|
	...
	while t_start <= end_range do
		...
		csv << [tz.utc_to_local(t_start)] + [count_msg] + [fsq_count] + (cat_parents.size>0 ? [array_to_csvstring(cat_parents.sort_by{|k,v| v}.reverse)] : [""]) + (cat.size>0 ? [array_to_csvstring(cat.sort_by{|k,v| v}.reverse)] : [""])
		t_start = t_end
	end
end