...
db = connection.db("citytweets")
berlintweets = db.collection("berlintweets")
rosenheimtweets = db.collection("rosenheimtweets")
reykjavik = db.collection("reykjaviktweets")
...
def addTweet(city_collection,status)
  if(status.geo!=nil)
...
client.locations(13.0882097323,52.3418234221,13.7606105539,52.6697240587) do |status|
  ...
  addTweet(berlintweets,status)
end
client.locations(12.077190,47.826542,12.155770,47.882568) do |status|
  addTweet(rosenheimtweets,status)
end
client.locations(-21.962940,64.120270,-21.843590,64.15644) do |status|
  addTweet(reykjaviktweets,status)
end